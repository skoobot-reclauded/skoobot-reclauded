...
| `KeyBind:formatKeyString` / `getBindTable` | — (#57) | E/KeyBind.lua:114 (getBindTable), :158 (formatKeyString) | ok | The engine renders a binding as `"SF7"` — one letter per modifier, no separator — which is what the Key Bindings screen shows; a message to the player reads better as `"Shift+F7"`. `getBindTable` is what "bound" means: the player's remap if there is one, else the keybind file's default. `src/data/keys.lua` `describe()` does the expansion; mouse and gesture bindings return nil there and fall back to the engine's formatter. |
| `KeyBind.binds_def` / `binds_remap` / `bindKeys()` | — (#50) | E/KeyBind.lua:33–41 (defs), :127–136 (rebuild) | ok | Every addon's `defineAction` lands in the **one class-level** `binds_def`, the player's remaps in `binds_remap`, and `bindKeys` rebuilds a handler's `binds` from both, remap over default. Read those two, never `game.key.binds`: that is built in `Game:init`/`loaded` and stays stale until UserChat's post-run `bindKeys()` (see the `addBinds` row). |
| two actions on one key string | — (#50) | E/KeyBind.lua:228–233 | **trap** | `receiveKey` fires whichever of them `pairs()` happens to yield first and has a handler, then returns: the other never sees the key, and **nothing is reported** — no error, no log line, no mark in the Key Bindings screen. The only way to see it is to compare `binds_def` and `binds_remap` yourself, which `src/data/keys.lua` `collisions()` does, advisory only; the addon never rebinds anything. |
