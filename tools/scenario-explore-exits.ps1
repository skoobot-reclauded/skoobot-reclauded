<#
    ...
    A  the level is exhausted and the player is NOT on an exit
       -> auto-explore should target an EXIT and keep running
    B  the same, with the player standing ON the down stairs
       -> whatever it does here is what #103 actually fires on
    C  the bot's own decision in state B, so the stop text is the real one

    A is asserted, since it is the claim #121 settled by reading and is worth
    a regression guard. B and C are measured and printed: asserting a value
