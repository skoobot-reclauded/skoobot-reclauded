...
'@
    ...
    Check ($m0 -match 'choices=\[a\) Talent rules -- which talents the bot may use \| b\) Stop conditions -- when it hands back \| c\) Settings -- thresholds, delay, popup, logging \| d\) Cancel\]') 'the four choices keep their letters (#95 added Settings as c)'
    ...
    Check ($m1 -match 'choices=\[a\) Talent rules -- which talents the bot may use \| b\) Stop conditions -- when it hands back \| c\) Settings -- thresholds, delay, popup, logging \| d\) Cancel\]') 'the choices are unchanged'
