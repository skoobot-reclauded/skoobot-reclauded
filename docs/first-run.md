...
|---|---|---|
| Low Health Ratio | 0.5 | A fraction of your life pool (0.5 is half) -- your maximum life, plus whatever keeps you alive below zero, which is what the game itself measures. While enemies are in view, the bot stops when life is below it. The other life thresholds follow from it: losing half of this fraction in one turn is the Big Loss stop; in a fight, losing a quarter of it in one turn uses a Damage Prevention talent, and missing a quarter of it uses a Recovery talent. |
| Ignore Damage Above Life Ratio | 0.9 | A fraction of your life pool (0.9 is nine tenths); see Low Health Ratio for what the pool is. While exploring with nothing in view, damage is ignored as long as life stays above it, so a single poison tick does not stop the bot; once life is below it, any damage taken while exploring hands back. It is also the scale that stop is measured on: life exactly at this ratio is threat 1, and twice as far below it is threat 2. See Maximum Enemy Power. |
